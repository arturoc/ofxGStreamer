...
if [ "$OF_BRANCH" == "stable" ]; then
    sed -i "s/$(CXX) -c $(OPTIMIZATION_CFLAGS) $(CFLAGS) $(CXXFLAGS) $(OF_CORE_INCLUDES_CFLAGS) -MMD/$(CXX) -c $(OPTIMIZATION_CFLAGS) $(CFLAGS) $(CXXFLAGS) $(OF_CORE_INCLUDES_CFLAGS) $(ADDON_INCLUDE_CFLAGS) -MMD/g;" ~/openFrameworks/libs/openFrameworksCompiled/project/makefileCommon/compile.project.mk
fi
